...
var Alerts = (function($, window, document, undefined) {
	'use strict'; //alerts returned from web service

	var allAlerts = []; //array to hold route list and references to alerts

	var alertsByRoute = [];
	var singleRoute = false;
	var serviceHostUrl = $('meta[name=web-service-uri]').attr('content'); //build the array list of alerts ordered by route

	var buildAlerts = function buildAlerts(routeAbbr) {
		...
		allAlerts.forEach(function(alert, index) {
			...
			alert.informed_entity.forEach(function(entity, idx) {
				...
				var current = $.grep(alertsByRoute, function(alert) {
					return alert.route_id === entity.route_id;
				})[0]; //if the route_id of the current informed_entity hasn't already been added to the
				...
				if (current === undefined) {
					...
					switch (entity.route_id) {
						...
						case '901':
							(sort = 1), (icon = 'blue-lrt');
							break;

						case '902':
							(sort = 2), (icon = 'green-lrt');
							break;
						...
						case '921':
							(sort = 4), (icon = 'gray-bus');
							break;

						case '923':
							(sort = 5), (icon = 'gray-bus');
							break;

						case '992':
							(sort = 6), (icon = 'gray-bus');
							break;

						case '888':
							(sort = 7), (icon = 'gray-outline-train');
							break;
						//add 10 to the rest so routes 2,3,4,etc will follow the METRO and rail

						default:
							sort = parseInt(entity.route_id) + 10;
					...
					alertsByRoute.push({
						...
						route_label: entity.route_label,
						icon: icon,
						alert_index: [index],
					});
				} else {
	...
	var outputAlertsForRoute = function outputAlertsForRoute(routeAbbr) {
				...
				alertList.append(
					$('<p/>').append(
						$('<a/>', {
							...
							.attr('data-index', alertIndex)
							.text(
								allAlerts[alertIndex].header_text.translation[0]
									.text
							)
	...
	var outputAlerts = function outputAlerts() {
		//first sort the alertsByRoute array by the sort_order assigned in the previous function
		alertsByRoute.sort(function(a, b) {
			a = a.sort_order;
			b = b.sort_order;
		...
		alertsByRoute.forEach(function(route, index) {
			var alertsDiv = $('<div/>', {
				class: 'accordion service-alerts',
			});
			var card = $('<div/>', {
				class: 'card',
			});
			var btn = $(
				'<button type="button" class="btn d-flex align-items-center btn-block text-left collapsed" data-toggle="collapse" data-target="#route' +
					route.route_id +
					'alerts" aria-expanded="false" />'
			);
			btn.append(
				$(
					'<img alt="" class="icon circle-blue-lrt" src="/img/svg/circle-' +
						route.icon +
						'.svg">'
				)
			);
			...
			btn.append(
				$('<div/>', {
					...
					.append(
						$('<span/>', {
							class: 'number-alerts',
						}).text(route.alert_index.length + ' Alert(s)')
					)
					.append(
						$('<img/>', {
							class: 'icon chevron-down-blue ml-2',
							src: '/img/svg/chevron-down-blue.svg',
						})
					)
			);
			card.append(
			...
			card.append(
				$('<div/>', {
					class: 'collapse',
					id: 'route' + route.route_id + 'alerts',
				}).append(alertList)
			);
	...
	var showAlertText = function showAlertText() {
		var idx = $(this).data('index');
		var alertHeading = $('<h3/>')
			.text(
				allAlerts[$(this).data('index')].header_text.translation[0].text
			)
			.prop('outerHTML');
		var routeList = $('<p/>')
			.append(
				$('<b/>').text(
					'For the following route(s): ' + getAlertRouteList(idx)
				)
			)
			.prop('outerHTML');
		var alertText = allAlerts[idx].description_text.translation[0].text;
		$('#alertText').html(
			alertHeading + routeList + alertText.replace(/\r\n/g, '<br/>')
		);
		$('#alertTextModal').modal({
			show: true,
		});
		return false;
	};

	var getAlertRouteList = function getAlertRouteList(idx) {
		var routeList = [];
		allAlerts[idx].informed_entity.forEach(function(route, i) {
			var rt = $.grep(alertsByRoute, function(obj) {
				return obj.route_id === route.route_id;
			})[0];

			if (
				!includesRoute(
					routeList,
					rt.route_label.replace('Route', '').replace('All s', 'All')
				)
			) {
				routeList.push({
					sort_order: rt.sort_order,
					route_label: rt.route_label
						.replace('Route', '')
						.replace('All s', 'All'),
				});
			}
		...
		return routeList
			.map(function(el) {
				return el.route_label;
			})
			.join(',');
	};

	var includesRoute = function includesRoute(arr, label) {
		for (var i = 0; i < arr.length; i++) {
			if (arr[i].route_label === label) {
				return true;
			}
		}
	...
	var getAlertsForRoute = function getAlertsForRoute(routeAbbr) {
		singleRoute = true;
	...
	var init = function init() {
		...
		if (allAlerts.length > 0) return; //get all the alerts, convert to Json object and pass to buildAlerts method

		$.get(serviceHostUrl + '/alerts/all').done(function(result) {
			allAlerts = JSON.parse(JSON.stringify(result));
			buildAlerts();
		});
	};
