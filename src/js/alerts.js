...
var Alerts = function ($, window, document, undefined) {
  ...
  var outputAlertsForRoute = function outputAlertsForRoute(routeAbbr) {
    ...
    alertsByRoute.forEach(function (route, index) {
      ...
      route.alert_index.forEach(function (alertIndex, idx) {
        alertList.append($('<p/>').append($('<a/>', {
          href: '#',
          id: 'this' + alertIndex + idx
        }).attr('data-index', alertIndex).text(allAlerts[alertIndex].header_text.translation[0].text).on('click', showAlertText)));
      });
  ...
  var outputAlerts = function outputAlerts() {
    ...
    alertsByRoute.forEach(function (route, index) {
      ...
      btn.append($('<div/>', {
        ...
        class: 'number-alerts'
      }).text(route.alert_index.length + ' Alert(s)')).append($('<img/>', {
  ...
  var showAlertText = function showAlertText() {
    ...
    $('#alertText').html(alertHeading + routeList + alertText.replace(/\r\n/g, '<br/>'));
  ...
  var getAlertRouteList = function getAlertRouteList(idx) {
    ...
    allAlerts[idx].informed_entity.forEach(function (route, i) {
      ...
      if (!includesRoute(routeList, rt.route_label.replace('Route', '').replace('All s', 'All'))) {
        routeList.push({
          sort_order: rt.sort_order,
          route_label: rt.route_label.replace('Route', '').replace('All s', 'All')
        });
      }
    });
    ...
    return routeList.map(function (el) {
  ...
  var getAlertsForRoute = function getAlertsForRoute(routeAbbr) {
    singleRoute = true;
    $.get(serviceHostUrl + '/alerts/' + routeAbbr).done(function (result) {
