var CoordinateConversion = (function() {
    ...
    */
	...
	function ArcLengthOfMeridian(phi) {
		...
		/* Precalculate gamma */
		gamma =
			(15.0 * Math.pow(n, 2.0)) / 16.0 +
			(-15.0 * Math.pow(n, 4.0)) / 32.0;

		/* Precalculate delta */
		delta =
			(-35.0 * Math.pow(n, 3.0)) / 48.0 +
	...
	function FootpointLatitude(y) {
		...
		gamma_ =
			(21.0 * Math.pow(n, 2.0)) / 16.0 +
			(-55.0 * Math.pow(n, 4.0)) / 32.0;

		/* Precalculate delta_ (Eq. 10.22) */
		delta_ =
			(151.0 * Math.pow(n, 3.0)) / 96.0 +
		...
		return $.Deferred(function(dfd) {
			...
			require([
				...
				if (mapType === 'full') {
					...
					mapLayers = [routesLayer, routestopLayer];
				} else if (mapType === 'trip') {
					mapLayers = [tripLayer, tripStopLayer];
				}
...
var BOM = (function($, window, document, undefined) {
	...
	var _STOPS_QUERY_LAYER =
		'https://arcgis.metc.state.mn.us/transit/rest/services/transit/BOM_Points/MapServer/0';
	var _ROUTE_SERVICE =
		'https://arcgis.metc.state.mn.us/transit/rest/services/transit/TRIM/MapServer';
				...
				if (response.length > 0) {
					...
					$.each(response, function() {
						...
						if (_ROUTESFORSTOP) {
							...
							if (
								_ROUTESFORSTOP.indexOf(
									this.RouteId + this.Terminal
								) > -1 ||
								_SHOWALLBUSES
							) {
								pnt = newPointFromLatLong(
									this.Latitude,
									this.Longitude
								); // create a point from the bus location LAT/LONG
								drawVehicleOnMap(
									pnt,
									this.RouteId,
									this.Terminal,
					...
					do {
						// if we're not zooming we will 'do' this just once
						// if we are zooming, the first time through we want to test the current extent -- so NO DELTA needed
						var z = 0;
						if (i > 0) {
							// let z = 0 on the first time through, after that the exponent is one less than the iteration
						...
						d === 0 &&
						_CURRENTLOD.level - i >= _MAP.getMinZoom()
					); // keep zooming out until bus is found or extent too big
				}
				var dt = new Date();
				var tm =
	...
	var updateBusesOnMap = function(/*boolean*/ zoomFlag) {
		...
		_BOMRUNNING = true;
		if (_TICKSTOP) {
			// we're keeping track of service update cycles
			_TICKS++;
			if (_TICKS > _TICKSTOP) {
				// pause the service after limit reached
				stopBusesOnMap();
				showMapBanner(
		...
		drawBuses(/*boolean*/ zoomOnce, function(busesShowing, busesTotal) {
			...
			} else if (busesTotal === 0) {
				// nothing to report
				if (_TICKSTOP) {
					showMapBanner(
						'Real-time data are not available</br>or there is no service.'
					);
					stopBusesOnMap();
				}
			}
	...
	var newPointFromXY = function(/*float*/ x, /*float*/ y) {
		...
		require(['esri/geometry/Point'], function(Point) {
			...
			p = new Point(
				CoordinateConversion.RadToDeg(parseFloat(latlng[1])),
				CoordinateConversion.RadToDeg(parseFloat(latlng[0]))
			);
		});
		return p;
	};
		...
		return $.Deferred(function(dfd) {
			$.ajax({
				...
				data: {
					...
					returnGeometry: true,
					outFields:
						'site_id, site_on, site_at, ROUTES, NROUTES, ROUTEDIRS, ROUTETERMDIRS',
					f: 'json',
				},
				dataType: 'json',
			})
				.done(function(r) {
					...
					if (r.error) {
						...
						if (r.features.length > 0) {
							...
							var feature = r.features[0];
							stopPoint = newPointFromXY(
								feature.geometry.x,
								feature.geometry.y
							); //convert to UTM result to geographic LNG/LAT
							...
							var work,
								routesForStop = []; // Route + Term + Dir -- we use this list to match to actual bus identifiers
							...
							if (typeof feature.attributes !== 'undefined') {
								...
								if (
									feature.attributes.hasOwnProperty(
										'ROUTETERMDIRS'
									) &&
									feature.attributes['ROUTETERMDIRS'] !== ''
								) {
				...
				function(r) {
					if (r.error) {
						...
						if (r.features.length > 0) {
							...
							console.warn(
								'route query returned no results: ' + queryWhere
							);
							dfd.reject();
						}
					}
